...
contract('CardFactory', (accounts) => {
  ...
  it('should fail to change mintableToken — not a owner', async () => {
    try {
      await factory.setMintableToken("0x0000000000000000000000000000000000000000", {from: account[1]});
    } catch (e) {
      console.log('Reverted!');
    }
  });
  ...
  it('should change mintableToken', async () => {
    await factory.setMintableToken("0x0000000000000000000000000000000000000000");
    assert.equal(await factory.mintableToken(), "0x0000000000000000000000000000000000000000");
  });
})